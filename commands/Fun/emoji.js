...
module.exports = class extends Command {
	...
	async run(message, [ text ]) {
		const maxEmbed = this.client.helpers.Miscs.getEmbed({ color: message.guild.settings.embedcolor, footer: false });
		maxEmbed.setDescription(message.language.get('MAX_CHARS'));
		if (text.length > 25) return message.channel.send({ embed: maxEmbed });
		const minEmbed = this.client.helpers.Miscs.getEmbed({ color: message.guild.settings.embedcolor, footer: false });
		...
		const generalEmbed = this.client.helpers.Miscs.getEmbed({ color: message.guild.settings.embedcolor, footer: false });
