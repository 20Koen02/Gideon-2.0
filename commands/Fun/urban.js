...
module.exports = class extends Command {
	...
	async run(message, args) {
		try {
			...
			if (urban.example != '') {
				const defEmbed = this.client.helpers.Miscs.getEmbed({ color: message.guild.settings.embedcolor, footer: false });
				defEmbed
				...
				const defXEmbed = this.client.helpers.Miscs.getEmbed({ color: message.guild.settings.embedcolor, footer: false });
				defXEmbed.addField(message.language.get('URBAN_DEFINITION', capitalize(urban.word)), `${capitalize(urban.definition)}`);
				return message.channel.send({ embed: defXEmbed });
			}
		} catch (e) {
			// eslint-disable-line no-unused-vars
			const noDefEmbed = this.client.helpers.Miscs.getEmbed({ color: message.guild.settings.embedcolor, footer: false });
