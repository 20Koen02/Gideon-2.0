...
class PlayCommand extends Command {
    ...
    async run(msg, song) {
        ...
        const songChoices = [
            "Please type the number of the song you want to be played (1-5). Type `cancel` to exit adding the song.",
            ""
        ];
        ...
        try {
            ...
            if (msg.guild.music.status == "paused" || msg.guild.music.status == "playing")
                return msg.send(`I added song **${song.title}** to the queue!`);
    ...
    fetchFromURL(url) {
        const youtubeMatch = /(?:http(?:s)?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:(?:watch)?\?(?:.*&)?v(?:i)?=|(?:embed|v|vi|user)\/))([^?&"'<> #]+)/gi.exec(
            url
        );
