...
class Miscs {
	...
	getEmbed(options = {}) {
		...
		if (!options.hasOwnProperty('color')) options.color = 0x36393e;
		if (options.text && typeof options.text !== 'string') throw 'Text must be string.';

		const embed = new MessageEmbed().setColor(options.color || 0x00aaff);
		if (options.footer === true) embed.setFooter(`© ${this.client.user.username} 2019${options.text ? ` | ${options.text}` : ''}`);
		return embed;
	}
	...
	request(URL, method = 'get') {
		return request[method](URL).set('User-Agent', `[Discord BOT] (${this.client.user.username}-v${version})`);
	}
}
