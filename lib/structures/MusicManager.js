...
class MusicManager {
    ...
    join(voiceChannel) {
        return voiceChannel.join().catch(err => {
            if (String(err).includes("ECONNRESET"))
                throw "There was an issue connecting to the voice channel, please try again.";
    ...
    async leave() {
        if (!this.voiceChannel)
            throw "I already left the voice channel! You might want me to be in one in order to leave it...";
    ...
    async play() {
        ...
        if (!this.connection) {
            ...
            if (!this.connection) throw "Error. Try again later...";
        }
