...
class GideonConsole extends Console {
    constructor(client) {
    ...
    write(data, type = "log") {
        ...
        data = GideonConsole.parseData(data);
        const { time, message } = GideonConsole.COLORS[type];
        const timestamp = time(`[${this.timestamp}]`);
        super[GideonConsole.TYPES[type] || "log"](data.split("\n").map(str => `${timestamp} ${message(str)}`).join("\n"));
    }
    ...
    static parseData(data) {
        if (Array.isArray(data)) return data.map(GideonConsole.parseData).join("\n");
